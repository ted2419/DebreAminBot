...
def save_progress(user_id, course, progress):
    if user_id not in PROGRESS:
        PROGRESS[user_id] = {}
    PROGRESS[user_id][course] = progress
    SHEET.append_row([user_id, course, progress])  # Log to Google Sheets
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Welcome to Debre Amin Sunday School Bot! Choose an option:", reply_markup=reply_markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'courses':
        ...
        await query.edit_message_text(f"Course: {course}\nProgress: {progress}\nUse /update_progress to change.", parse_mode='Markdown')
    ...
    elif query.data == 'add_course' and user_id in ADMIN_IDS:
        await query.edit_message_text("Enter new course name with /add_course <name>")
    elif query.data == 'upload_file' and user_id in ADMIN_IDS:
        await query.edit_message_text("Send a PDF file (max 50MB) to upload.")
